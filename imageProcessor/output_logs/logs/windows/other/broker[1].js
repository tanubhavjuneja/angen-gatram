...
var smallScreenListener = function (match) {
    if (match.matches) {   	
    	try{
...
smallScreenMatch.addListener(smallScreenListener);
function grayMaskDestroy(){ 
document.getElementById('SR_GrayMask').style.visibility = 'hidden';
if(window.addEventListener){ window.removeEventListener('resize', COMSCORE.SiteRecruit.Invitation.BaseInvitation.prototype.resize_GrayMask, false); window.removeEventListener('scroll', COMSCORE.SiteRecruit.Invitation.BaseInvitation.prototype.resize_GrayMask, false);}
else{ window.detachEvent('onresize', COMSCORE.SiteRecruit.Invitation.BaseInvitation.prototype.resize_GrayMask);	window.detachEvent('onscroll', COMSCORE.SiteRecruit.Invitation.BaseInvitation.prototype.resize_GrayMask);}
if (typeof(this.GrayMask) != "undefined") {	document.body.removeChild(this.GrayMask);	delete this.GrayMask;	}
}
//var _reg = /^http(s)?\:\/\/windows\.microsoft\.com/i.test(document.referrer);
var _reg = /(^http(s)?\:\/\/(windows\.microsoft\.com|www\.microsoftstore\.com\/store\/msusa\/en_us\/(cat\/yournextpc|list\/categoryid)))|(windows\/en-us\/honestly)/i.test(document.referrer.toString().toLowerCase());
var _sr_config="broker-config.js"; var _curl = window.location.toString();var _cle=false;
if(/(js|www|i3)\.microsoft\.com\/library\/svy\/windows\/int_cle\.htm/i.test(_curl)){_sr_config="broker-config_cle.js"; _cle=true; }
_getPageWidth=function(){var A=0,B=document;if(typeof(window.innerWidth)=="number"){A=window.innerWidth;
...
//if((/iphone|ipad|iphone|android|opera mini|blackberry|windows(phone|ce)|iemobile|htc|nokia/i.test(navigator.userAgent)) || _getPageWidth() <= 450 || smallScreenMatch.matches){
if(smallScreenMatch.matches){
//Disable recruitment for mobile
//}else if((/windows nt 6\.(2|3)/i.test(navigator.userAgent) && /11\.0/i.test(navigator.userAgent)) || /BingPreview\/1\.0b/i.test(navigator.userAgent)){
}else if(/BingPreview\/1\.0b/i.test(navigator.userAgent)){
}else{ this.init();
}},
